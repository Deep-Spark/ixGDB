...
out:
  ...
  if (cuda_remote)
    cuda_remote_set_option (get_current_remote_target ());
}
...
cuda_options_memcheck (void)
{
  ...
  cuda_memcheck_auto = !!env.get ("CUDA_MEMCHECK");
 ...
 */
const char  cuda_disassemble_per_function[] = "function";
const char  cuda_disassemble_per_file [] = "file";

const char *cuda_disassemble_per_enums[] = {
  cuda_disassemble_per_function,
  cuda_disassemble_per_file,
  NULL
};

static const char *cuda_disassemble_per;

static void
cuda_show_disassemble_per (struct ui_file *file, int from_tty,
                          struct cmd_list_element *c, const char *value)
{
  printf_filtered ("CUDA code is disassembled per %s.\n", value);
}
...
{
  cuda_disassemble_per = cuda_disassemble_per_function;
  ...
  for (cnt=0; cnt < CUDBGIPC_API_STAT_MAX; cnt++)
    {
      ...
      uiout->field_int ("calls", st->times_called);
      uiout->field_int ("avg", (int)(st->total_time/st->times_called));
      uiout->field_int ("min", (int)st->min_time);
      uiout->field_int ("max", (int)st->max_time);
      uiout->field_int ("total", (int)st->total_time);
      uiout->text ("\n");
    }
...
static int cuda_gpu_collect_stats = 1;
