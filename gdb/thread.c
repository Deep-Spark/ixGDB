		     ...
		     int show_global_ids)
{
  ...
  bool focus_is_device = cuda_focus_is_device();
...
static void
switch_to_thread_1 (ptid_t ptid, thread_info *thr, int keep_cuda_focus)
{
  ...
  if (!cuda_focus_is_device () && inferior_ptid == ptid)
    return;

  /* If we switch the focus away from the CUDA device, we will end up
     with regular information from the host machine's threads.  If we
     currently have focus on the device, make it so we keep that focus
     regardless of whether we switch away from the original thread or not.  */

  /* CUDA - coredumps */
  if (ptid != null_ptid)
    {
      struct inferior *inf = find_inferior_pid (ptid.pid ());
  ...
  if (thr)
    {
	 ...
	 internal event.  */
	  ...
	  && !is_exited (ptid)
	  && !is_executing (ptid))
	thr->suspend.stop_pc = regcache_read_pc (get_thread_regcache (ptid));
      else
	thr->suspend.stop_pc = ~(CORE_ADDR) 0;
    }
}
...
{
  gdb_assert (thr != NULL);
  switch_to_thread_1 (thr->ptid, thr, 0);
}
...
{
  gdb_assert (thr);
  switch_to_thread_1 (thr->ptid, thr, 1);
}
...
scoped_restore_current_thread::scoped_restore_current_thread ()
{
  ...
  m_cuda_coords = CUDA_INVALID_COORDS;

  /* CUDA - get current focus */
  cuda_coords_get_current (&m_cuda_coords);
