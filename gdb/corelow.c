...
core_target_open (const char *arg, int from_tty)
{
  ...
  expanded = fname ? tilde_expand (fname) : NULL;
  gdb::unique_xmalloc_ptr<char> filename (expanded);
  if (filename.get() && !IS_ABSOLUTE_PATH (filename.get ()))
    filename.reset (concat (current_directory, "/",
			    filename.get (), (char *) NULL));

  expanded = cname ? tilde_expand (cname) : NULL;
  gdb::unique_xmalloc_ptr<char> cudacorename (expanded);
  if (cudacorename.get () && !IS_ABSOLUTE_PATH (cudacorename.get ()))
    cudacorename.reset (concat (current_directory, "/",
				cudacorename.get (), (char *) NULL));
...
{
  /* This workaround is to avoid to call two time intialize_corelow and is
     is linked to the construction of the init.c */
  /* When ENABLE_QNX_TARGET is on, the function initialize_corelow is also
     called by the function _initalize_nto_tdep (nto-tdep.c) but this last
     function is also called in the init.c.
     init.c is generated by the build system by using 'sed' and listing all
     functions which name begin by _initialize. So renaming the function
     depending the target is useless since sed don't care about the
     if/ifdef preprocessors.  */
#ifndef __QNXTARGET__
  initialize_corelow ();
#endif
}
