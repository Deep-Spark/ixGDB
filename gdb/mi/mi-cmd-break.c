...
mi_cmd_break_insert_1 (int dprintf, const char *command, char **argv, int argc)
{
  ...
  if (condition != NULL)
    {
      char *new_extra = (char *) xmalloc (1024);
      make_cleanup (xfree, new_extra);

      if (thread > -1)
	snprintf (new_extra, 1024, "if %s thread %d", condition, thread);
      else
	snprintf (new_extra, 1024, "if %s", condition);
      parse_extra = true;
      extra_string = new_extra;
      condition = NULL;
      thread = -1;
    }
