...
public:
  ...
  virtual ptid_t wait (ptid_t arg0, struct target_waitstatus *arg1,
		       int TARGET_DEBUG_PRINTER () arg2) override;
  ...
  virtual bool has_execution (ptid_t arg0) override;

  virtual const char *pid_to_str (ptid_t arg0) override;
  ...
  virtual int follow_fork (int arg0, int arg1) override;
  ...
  while (true)
    {
      ...
      if (length == 0)
        {
          error ("Failed to receive a packet");
        }
      if (length == -1 || current_session->channelrd != SET_CHANNEL_TEXT)
        {
          break;
        }
...
{
  return qnx_getpkt (get_current_remote_target (), buf, forever);
}
...
qnx_remote_target<parent>::open (const char *name, int from_tty)
{
  ...
  push_target (remote);	/* Switch to using remote target now.  */
  nto_add_commands ();
...
template <class parent>
ptid_t
qnx_remote_target<parent>::wait (ptid_t ptid, struct target_waitstatus *status, int i)
{
...
//    priv->starting_ip = stopped_pc;
	ti = add_thread_with_info (ptid_t (pid, 0, tid), priv);
	...
	status->value.related_pid = ptid_t (inf_data->child_pid, 0, 1);
	inf_data->vfork
...
template <class parent>
bool
qnx_remote_target<parent>::has_execution (ptid_t ptid)
{
  struct inferior *inf;
...
template <class parent>
int
qnx_remote_target<parent>::follow_fork (int follow_child, int detach_fork)
{
  ...
  if (follow_child)
      ...
      if (nto_inferior_data (parent_inf)->vfork)
	{
	  ...
	  inferior_ptid = ptid_t (child_pid, 0, 1);
      ...
      if (!detach_fork)
	{
	  ...
	  inferior_ptid = nto_get_thread_alive (this, ptid_t (child_pid, 0, 1));
	  add_thread (inferior_ptid);
