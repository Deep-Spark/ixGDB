  ...
  if (!val->contents)
    {
      ...
      val->contents.reset ((gdb_byte *) xzalloc (val->length));
    }
}
...
{
  if (!target_float_is_valid (value_contents (val), value_type (val)))
    error (_("Invalid floating value found in program."));

  return target_float_to_host_double (value_contents (val), value_type (val));
}
  ...
  if (code == TYPE_CODE_FLT)
      ...
      if (!target_float_is_valid (valaddr, type))
	{
	  *invp = 1;
      ...
      return target_float_to_host_double (valaddr, type);
    }
				 ...
				 CORE_ADDR address)
{
  ...
  if (valaddr == NULL)
  ...
  else
    {
      ...
      v->length = std::min (TYPE_LENGTH (type), length);
...
struct value *
value_from_double (struct type *type, double num)
{
  struct value *val = allocate_value (type);
  ...
  if (code == TYPE_CODE_FLT)
    {
      target_float_from_host_double (value_contents_raw (val), base_type, num);
    }
  else
...
value_fetch_lazy_register (struct value *val)
{
  ...
  value_contents_copy (val, value_embedded_offset (val),
		       ...
		       std::min (value_length (val), value_length (new_val)));
