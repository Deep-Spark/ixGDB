...
#ifndef GDBSERVER
  if (cuda_remote)
    cuda_remote_notification_aliased_event (get_current_remote_target ());
#endif
...
#ifndef GDBSERVER
  if (cuda_remote)
    return cuda_remote_notification_pending (get_current_remote_target ());
#endif
...
#ifndef GDBSERVER
  if (cuda_remote)
    return cuda_remote_notification_received (get_current_remote_target ());
#endif
...
cuda_notification_analyze (ptid_t ptid, struct target_waitstatus *ws, int trap_expected)
{
  ...
  if (cuda_remote)
    {
#ifdef __QNXTARGET__
      cuda_remote_notification_analyze (get_current_remote_target (), ptid, ws);
#else /* __QNXTARGET__ */
      cuda_remote_notification_analyze (get_current_remote_target (), ptid);
#endif /* __QNXTARGET__ */
      return;
    }
#endif
  ...
  if (cuda_notification_info.sent &&
      ...
      (ws->value.sig == GDB_SIGNAL_URG ||
#ifdef __QNXHOST__
       ws->value.sig == GDB_SIGNAL_EMT ||
...
cuda_notification_mark_consumed (void)
{
  ...
  if (cuda_remote)
    {
      cuda_remote_notification_mark_consumed (get_current_remote_target ());
      return;
    }
#endif
...
cuda_notification_consume_pending (void)
{
  ...
  if (cuda_remote)
    {
      cuda_remote_notification_consume_pending (get_current_remote_target ());
      return;
    }
#endif
