		         ...
		         LONGEST (*expr_evaluate)(void*, CORE_ADDR, void*))
{
  ...
  TYPE_BOUND_BATON_FUNCTION (result_type) = (void *) expr_evaluate;
...
struct type *
create_static_range_type_d (struct type *result_type, struct type *index_type,
			    LONGEST low_bound,
		     	    LONGEST high_bound,
		     	    void *dwarf_low, 
		     	    ...
		     	    LONGEST (*expr_evaluate)(void*, CORE_ADDR, void*))
{
  ...
  return create_range_type_d_pgi (result_type, index_type, &low,
				  &high, 0, 0, 0, dwarf_low, dwarf_high,
				  dwarf_count, 0, 0, 0, expr_evaluate);
}
...
create_range_type_d (struct type *result_type, struct type *index_type,
		     ...
		     LONGEST (*expr_evaluate)(void*, CORE_ADDR, void*))
{
  return create_range_type_d_pgi (result_type, index_type, low_bound,
				  high_bound, 0, 0, 0, dwarf_low, dwarf_high,
				  dwarf_count, 0, 0, 0, expr_evaluate);
}
...
create_range_type (struct type *result_type, struct type *index_type,
		   ...
		   const struct dynamic_prop *high_bound)
{
  return create_range_type_d (result_type, index_type, low_bound, high_bound,
			      NULL, NULL, NULL, NULL);
}
		     ...
		     htab_t copied_types)
{
  ...
  *TYPE_MAIN_TYPE (new_type) = *TYPE_MAIN_TYPE (type);
  ...
  if (objfile->cuda_objfile && !objfile->discarding)
    {
      TYPE_OBJFILE_OWNED (new_type) = 1;
      TYPE_OWNER (new_type).objfile = objfile;
    }
  else
    {
...
find_target_type_with_instance_flags (struct type *type)
{
  ...
  int mask = 0xFFFFFC00; /* Mask the first 10 bits */
  ...
  do {
    if ((TYPE_INSTANCE_FLAGS(type) & mask)
	== (TYPE_INSTANCE_FLAGS(cur_target) & mask))
      return cur_target;
