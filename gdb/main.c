...
#endif
  std::string val_1_str (std::to_string(1));
  if ((0 != setenv("DRT_WRAPGDBLAYER", val_1_str.c_str(), 1)) )
  {
    printf("Failed to configure the DRT_WRAPGDBLAYER environment variable.\n");
  }
  if ((0 != setenv("CUDA_LAUNCH_BLOCKING", val_1_str.c_str(), 1)) )
  {
    printf("Failed to configure the CUDA_LAUNCH_BLOCKING environment variable.\n");
  }
