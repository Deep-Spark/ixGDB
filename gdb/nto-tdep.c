...
extern void initialize_corelow (void);
...
{
  return (struct nto_gdbarch_data *) gdbarch_data (gdbarch, nto_gdbarch_data_handle);
}
...
nto_get_siginfo_type (struct gdbarch *gdbarch)
{
  ...
  pid_type = arch_type (gdbarch, TYPE_CODE_TYPEDEF,
			TYPE_LENGTH (int_type) * TARGET_CHAR_BIT, "pid_t");
  TYPE_TARGET_TYPE (pid_type) = int_type;
  TYPE_TARGET_STUB (pid_type) = 1;

  /* uid_t */
  uid_type = arch_type (gdbarch, TYPE_CODE_TYPEDEF,
			TYPE_LENGTH (uint_type) * TARGET_CHAR_BIT, "uid_t");
  TYPE_TARGET_TYPE (uid_type) = int_type;
  TYPE_TARGET_STUB (uid_type) = 1;

  /* clock_t */
  clock_type = arch_type (gdbarch, TYPE_CODE_TYPEDEF,
			  TYPE_LENGTH (uint_type) * TARGET_CHAR_BIT, "clock_t");
...
#if 0
static char *
nto_gdbarch_core_pid_to_str (struct gdbarch *gdbarch, ptid_t ptid)
{
