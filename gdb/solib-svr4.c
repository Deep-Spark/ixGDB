...
# endif
      /* Always get l_path and stuff it in so_original_name. */
	{
	  ...
	  target_read_string (li->l_path, &pathbuff,
			      PATH_MAX - 1, &err);
...
#ifdef __ANDROID__
static int
svr4_android_find_and_open_solib (const char *soname, unsigned flags, gdb::unique_xmalloc_ptr<char> *temp_name)
{
  char *ldpath, *sep;
