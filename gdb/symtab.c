	  ...
	  if (real_path != NULL)
	    {
	      const int realpath_len = strlen (real_path);
              const int dirname_len = SYMTAB_DIRNAME (s) ? strlen(SYMTAB_DIRNAME (s)) : 0;
	      ...
	      if (FILENAME_CMP (real_path, fullname) == 0)
		{
		 ...
		 with s->filename */
	      if (SYMTAB_DIRNAME (s) != NULL && s->filename != NULL &&
		  realpath_len == dirname_len + filename_len + 1 && 
		  filename_ncmp (real_path, SYMTAB_DIRNAME (s), dirname_len) == 0 && 
...
{
  gsymbol->cuda_name = (char *) obstack_alloc (&objfile->objfile_obstack, len + 1);
			     ...
			     const domain_enum domain)
{
  ...
  if (main_objfile)
	...
	fprintf_unfiltered (gdb_stdlog, "cuda_lookup_symbol_in_block(%s, %d, %s) = %s\n",
			    ...
			    block_index,
			    result.symbol ? SYMBOL_LINKAGE_NAME (result.symbol) : "NULL",
			    host_address_to_string (result.symbol));
      return result;
    }
      ...
      if (result.symbol)
	{
	  if (symbol_lookup_debug)
	    fprintf_unfiltered (gdb_stdlog, "cuda_lookup_symbol_in_block(%s, %d, %s) = %s\n",
				objfile_debug_name (main_objfile),
				block_index,
				SYMBOL_LINKAGE_NAME (result.symbol),
				host_address_to_string (result.symbol));
	  return result;
	}
    }
...
lookup_static_symbol (const char *name, const domain_enum domain)
{
  ...
  /* Check the CUDA objfile STATIC blocks first */
  for (objfile *objfile : current_program_space->objfiles ())
    {
      if (!objfile->cuda_objfile)
	continue;
      
      result = cuda_lookup_symbol_in_objfile (objfile, STATIC_BLOCK, name, domain);
      if (result.symbol != NULL)
	{
  ...
  for (objfile *objfile : current_program_space->objfiles ())
    {
      /* Skip CUDA objfiles as they were checked above */
      if (objfile->cuda_objfile)
	continue;
		      ...
		      const domain_enum domain)
{
  ...
  /* CUDA - symbols */
  result = cuda_lookup_symbol_in_block (objfile, GLOBAL_BLOCK, name, domain);
  if (result.symbol != NULL)
    return result;
      ...
      for (compunit_symtab *cust : obj_file->compunits ())
	{
	  ...
	  b = BLOCKVECTOR_BLOCK (bv, GLOBAL_BLOCK);
          ...
          if (obj_file->cuda_objfile)
            {
              const struct blockvector *found = blockvector_for_pc_sect (pc, section, &b,
                                                                   cust);
              if (!found)
                continue;
            }
...
static void info_main_command(const char* arg, int from_tty)
{
  printf_unfiltered("%s\n", main_name());
}
