/* DWARF 2 debugging format support for GDB.
   ...
   Copyright (C) Allinea Software Ltd. 2013
...
out:
  if (name && cu->language_defn 
      && (cu->language_defn->la_case_sensitivity == case_sensitive_off)) 
    {
      ...
      len = strlen (name);
      copy = (char *) obstack_alloc (&cu->comp_unit_obstack, len + 1);
      for (i= 0; i < len; i++)
                    ...
                    while (grandchild && grandchild->tag)
                      {
                        switch (grandchild->tag)
                          ...
                          case DW_TAG_subprogram:
                            ...
                            best_low = std::min (best_low, child_low);
                            best_high = std::max (best_high, child_high);
                            break;
                          }
...
read_structure_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  {
    type = dwarf2_init_float_type (objfile, 16, name, name);
    TYPE_NAME (type) = name;
...
make_loclist_baton (struct dwarf2_cu *cu, struct dwarf_block *blk)
{
    ...
    baton = (struct dwarf2_loclist_baton *) obstack_alloc (&cu->per_cu->dwarf2_per_objfile->objfile->objfile_obstack,
  ...
  if (attr || dwarf2_attr (die, DW_AT_lbase, cu)) 
    {
        ...
        baton_holder = OBSTACK_ZALLOC (&cu->per_cu->dwarf2_per_objfile->objfile->objfile_obstack,
        ...
        if (attr && attr_form_is_block (attr))
            {
                ...
                struct attribute *attr_alloc = dwarf2_attr(die, DW_AT_allocated, cu);
                if (attr_alloc)
        ...
        struct attribute *attr_pgi = dwarf2_attr (die, DW_AT_lbase, cu);
                          ...
                          enum type_code refcode)
{
  ...
  if (TYPE_CODE(target_type) == TYPE_CODE_VOID) {
    type = lookup_pointer_type (target_type);
  } else {
...
read_tag_string_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (attr)
    {
       if (attr->form == DW_FORM_block1) 
 	{
	  ...
	  baton->size = size + 1;
 	  ...
 	  baton->data = (const gdb_byte *) obstack_alloc (&cu->per_cu->dwarf2_per_objfile->objfile->objfile_obstack,
							  baton->size);
  ...
  index_type = objfile_type (objfile)->builtin_int;
  range_type = create_static_range_type_d (NULL, index_type, 1, length, NULL, NULL, len_compute, (LONGEST (*)(void*, CORE_ADDR, void*)) dwarf2_evaluate_int);
...
read_subroutine_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  switch (TYPE_CALLING_CONVENTION (ftype))
	...
	/* Set this subroutine as the "main" subroutine
	   for the program. */
	if (TYPE_NAME (ftype) != NULL)
	  set_objfile_main_name (objfile, TYPE_NAME (ftype), cu->language);
	break;
      case DW_CC_normal:
      case DW_CC_nocall:
	default:
		  ...
		  if (addr_class)
		    {
			  ...
			  switch (addr_class)
			    ...
			    default:
			      ...
			      complaint (_("Unsupported DW_AT_address_class %d"), addr_class);
			      ((gdb_byte *) baton->data)[0] = DW_OP_lit6;
			      break;
			    }
...
read_subrange_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (!low_compute && !high_compute
      && low.kind == PROP_CONST && high.kind == PROP_CONST
      && low.data.const_val > (high.data.const_val + 1))
    {
      if (cu->producer && strstr (cu->producer, "DBG_GEN"))
	{
	  /* Sun FORTE says 255 for -1; 254 for -2, etc.. yuk! */
	  low.data.const_val |= (-1 ^ 0xff);
	}
      else 
	{
	  low.data.const_val = high.data.const_val;
	}
    }
			...
			const struct abbrev_info &abbrev, const gdb_byte *info_ptr)
{
  ...
  for (i = 0; i < abbrev.num_attrs; ++i)
	...
	case DW_AT_name:
	  switch (tag)
		...
		struct objfile *objfile = dwarf2_per_objfile->objfile;
                ...
                if (!name || cu->language == language_fortran)
	...
	case DW_AT_MIPS_linkage_name:
	  /* Note that both forms of linkage name might appear.  We
	     ...
	     one we see.  */
	  if (cu->language == language_ada || !name)
	    name = DW_STRING (&attr);
  ...
  if (main_subprogram)
    set_objfile_main_name (dwarf2_per_objfile->objfile,
			   linkage_name ? linkage_name : name,
			   cu->language);
  ...
  for (;;)
      ...
      for (i = 0; i < die->num_attrs; ++i)
	{
          ...
          if (die->attrs[i].name == DW_AT_abstract_origin &&
              (int) dwarf2_get_ref_die_offset (&die->attrs[i]) == 0)
            continue;
...
#ifdef CUDA_DEBUG_LINE_EXTENSION
  const gdb_byte *header_end = line_ptr + lh->header_length;
#endif
...
#ifdef CUDA_DEBUG_LINE_EXTENSION
  /* explicitly defined as a unsigned 4-byte offset */
  if (line_ptr < header_end)
    {
      dwarf2_read_section (dwarf2_per_objfile->objfile, &dwarf2_per_objfile->str);

      lh->debug_str_offset = (uint32_t) bfd_get_32 (abfd, line_ptr);
      line_ptr += 4;
      if (dwarf_line_debug)
	printf_filtered ("debug_str_offset %u\n", lh->debug_str_offset);
      lh->debug_str_base = dwarf2_per_objfile->str.buffer + lh->debug_str_offset;
    }
#endif
...
private:
  ...
  std::vector<int> m_context_stack;
#endif
};
		     ...
		     ULONGEST context)
{
  ...
  if (dwarf_line_debug)
	...
	{
	  fprintf_unfiltered (gdb_stdlog,
			      "Recording line %u, file %s, address %s inline %s (context %ld)\n",
			      line,
  ...
  if (cu != nullptr)
    cu->get_builder ()->record_line (subfile, line, addr, inline_function, context);
}
#else
static void
dwarf_record_line_1 (struct gdbarch *gdbarch, struct subfile *subfile,
...
#ifdef CUDA_DEBUG_LINE_EXTENSION
  dwarf_record_line_1 (gdbarch, subfile, 0, address, cu, inline_function_name, context);
#else
...
lnp_state_machine::record_line (bool end_sequence)
{
  ...
  if (dwarf_line_debug)
    {
      fprintf_unfiltered (gdb_stdlog,
			  ...
			  " address %s, is_stmt %u, discrim %u context %d end_sequence %d\n",
			  m_line, to_underlying (m_file),
			  paddress (m_gdbarch, m_address),
			  m_is_stmt, m_discriminator, m_context, end_sequence);
    }
#else
  if (dwarf_line_debug)
    {
  ...
  if (end_sequence)
    ...
    m_context_stack.push_back (m_line);
  ...
  if (m_context && (m_context <= m_context_stack.size()))
    {
      ...
      context_line = m_context_stack[m_context - 1];
    }
#endif
  ...
  while (line_ptr < line_end)
      ...
      while (line_ptr < line_end && !end_sequence)
	{
	  ...
	  if (op_code >= lh->opcode_base)
	    ...
	    default:
	      {
		/* Unknown standard opcode, warn and ignore it */
		      ...
		      const char *dirname)
{
  ...
  if (cu)
    ...
    get_buildsym_compunit ()->start_subfile (filename);
                               ...
                               struct dwarf2_cu *cu, bool is_variable)
{
  ...
  sti.objfile = cu->per_cu->dwarf2_per_objfile->objfile;
  ...
  if (sti.flags == TYPE_INSTANCE_FLAG_CUDA_GLOBAL && is_variable)
    {
      ...
      struct bound_minimal_symbol bmsym = lookup_minimal_symbol ( name, NULL, symtab ? SYMTAB_OBJFILE (symtab) : NULL);
  ...
  if (name)
	...
	sym = allocate_symbol (objfile);
      ...
      if (die->tag == DW_TAG_subprogram &&
          cuda_is_bfd_cuda(cu->per_cu->dwarf2_per_objfile->objfile->obfd) &&
          cu->language == language_cplus)
        {
      ...
      if (cu->producer && strncmp (cu->producer, "PGCC", 4) == 0)
        {
		  ...
		  message = xstrprintf (_("<recursive type in %s, CU 0x%lx, DIE 0x%lx>"),
...
cuda_dwarf2_addr_size (struct objfile *objfile)
{
  ...
  per_objfile = (struct dwarf2_per_objfile *) objfile_data (objfile, dwarf2_objfile_data_key);

  /* Approximate addr_size from cuda gdbarch if .debug_info section is missing */
  if (!per_objfile->info.buffer)
    return gdbarch_ptr_bit (cuda_get_gdbarch ()) / TARGET_CHAR_BIT;

  info_ptr = (gdb_byte *) per_objfile->info.buffer + offset;

  memset (&cu_header, 0, sizeof cu_header);
  read_comp_unit_head (&cu_header, info_ptr, &get_dwarf2_per_objfile (objfile)->info,
		       rcuh_kind::COMPILE);
...
cuda_decode_line_table (struct objfile *objfile)
{
  ...
  if (!dwarf2_per_objfile->line.s.section)
    return;
      
  dwarf2_per_cu_data *per_cu = create_cu_from_index_list (dwarf2_per_objfile,
							  &dwarf2_per_objfile->line,
							  0,
							  (sect_offset)0,
							  0);
  if (!per_cu)
  ...
  dwarf2_cu *cu = new dwarf2_cu (per_cu);
  if (!cu)
    return;
  cu->ancestor = nullptr;
  ...
  do
	...
	return;
      ...
      cu->reset_builder ();
      cu->start_symtab ("", NULL, 0);

      dwarf2_start_subfile (cu, "", NULL);

      dwarf_decode_lines (cu->line_header, "", cu, NULL, 0, 1);

      cu->get_builder ()->end_symtab (0, SECT_OFF_TEXT (objfile));

      for (int i = 0; i < cu->line_header->file_names.size (); i++)
	{
	  file_entry &fe = cu->line_header->file_names[i];

	  dwarf2_start_subfile (cu, fe.name, fe.include_dir (cu->line_header));

	  struct subfile *current_subfile = cu->get_builder ()->get_current_subfile ();
	  gdb_assert (current_subfile);

	  if (!current_subfile->line_vector)
	    continue;

	  /* fixup the global and static blocks if needed */
	  int nitems = current_subfile->line_vector->nitems;
	  if (!nitems)
	    continue;

	  CORE_ADDR sf_start = current_subfile->line_vector->item[0].pc;
	  CORE_ADDR sf_end = current_subfile->line_vector->item[nitems-1].pc;

	  struct compunit_symtab *cust = current_subfile->symtab->compunit_symtab;
	  gdb_assert (cust);

	  struct blockvector *bl = (struct blockvector *) COMPUNIT_BLOCKVECTOR (cust);
	  gdb_assert (bl);

	  struct block *global_block = BLOCKVECTOR_BLOCK (bl, GLOBAL_BLOCK);
	  gdb_assert (global_block);

	  if (sf_start && (sf_start < BLOCK_START (global_block)))
	    {
	      if (cuda_line_debug)
		{
		  fprintf_unfiltered (gdb_stdlog, "update GLOBAL %s sf_start 0x%lx -> 0x%lx\n",
				      current_subfile->name,
				      BLOCK_START (global_block),
				      sf_start);
		}
	      BLOCK_START (global_block) = sf_start;
	    }

	  if (sf_end && (sf_end > BLOCK_END (global_block)))
	    {
	      if (cuda_line_debug)
		{
		  fprintf_unfiltered (gdb_stdlog, "update GLOBAL %s sf_end 0x%lx -> 0x%lx\n",
				      current_subfile->name,
				      BLOCK_END (global_block),
				      sf_end);
		}
	      BLOCK_END (global_block) = sf_end;
	    }

	  struct block *static_block = BLOCKVECTOR_BLOCK (bl, STATIC_BLOCK);
	  gdb_assert (static_block);

	  if (sf_start && (sf_start < BLOCK_START (static_block)))
	    {
	      if (cuda_line_debug)
		{
		  fprintf_unfiltered (gdb_stdlog, "update STATIC %s sf_start 0x%lx -> 0x%lx\n",
				      current_subfile->name,
				      BLOCK_START (static_block),
				      sf_start);
		}
	      BLOCK_START (static_block) = sf_start;
	    }

	  if (sf_end && (sf_end > BLOCK_END (static_block)))
	    {
	      if (cuda_line_debug)
		{
		  fprintf_unfiltered (gdb_stdlog, "update STATIC %s sf_end 0x%lx -> 0x%lx\n",
				      current_subfile->name,
				      BLOCK_END (static_block),
				      sf_end);
		}
	      BLOCK_END (static_block) = sf_end;
	    }
	}

      line_offset += cu->line_header->total_length + 4;

    } while (line_offset < get_dwarf2_per_objfile (objfile)->line.size);
}
