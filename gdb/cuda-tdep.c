...
cuda_register_name (struct gdbarch *gdbarch, int regnum)
{
  ...
  if (regnum < device_num_regs)
    {
      snprintf (buf, sizeof (buf), "V%d", regnum);
      return buf;
    }
  ...
  if (cuda_uregnum_p (gdbarch, regnum))
    {
      snprintf (buf, sizeof (buf), "S%d", regnum - tdep->first_uregnum);
      return buf;
    }
...
cuda_is_bfd_cuda (bfd *obfd)
{
  ...
  return (obfd &&
         ...
         (elf_tdata(obfd)->elf_header[0].e_machine == EM_CUDA
          ||elf_tdata(obfd)->elf_header[0].e_machine == 0xF8));//ILUVATAR Machine 0xF8
}
...
#define STT_LOOS_ENTRY       10     // Lowest operating system-specific symbol type
...
{
  int type = ((elf_symbol_type *) sym)->internal_elf_sym.st_info & 0xf;
  ...
  /* break_on_launch */
  if (((elf_symbol_type *) sym)->internal_elf_sym.st_other == STO_CUDA_ENTRY || type == STT_LOOS_ENTRY )
    {
