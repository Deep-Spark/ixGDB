...
static enum type_instance_flag_value last_segment_type = (enum type_instance_flag_value) 0;
...
float_type_from_length (struct type *type)
{
  ...
  if (TYPE_LENGTH (type) == TYPE_LENGTH (builtin->builtin_half))
    type = builtin->builtin_half;
  else if (TYPE_LENGTH (type) == TYPE_LENGTH (builtin->builtin_float))
			    ...
			    struct format_data *fmtp)
{
  ...
  if (exp && *exp == '/')
    {
      exp++;
      *fmtp = decode_format (&exp, last_format, 0, (enum type_instance_flag_value) 0);
...
output_command (const char *exp, int from_tty)
{
  ...
  if (exp && *exp == '/')
    {
      exp++;
      fmt = decode_format (&exp, 0, 0, (enum type_instance_flag_value) 0);
...
data_address_info (const char *exp, int from_tty)
{
  ...
  fputs_filtered (paddress (get_type_arch (value_type (val)), value_address (val)), gdb_stdout);
  printf_filtered ("\n");
}

...
x_command (const char *exp, int from_tty)
{
  ...
  fmt.count = 1;
  fmt.raw = 0;
  fmt.segment_type = (enum type_instance_flag_value) 0;
  ...
  if (exp && *exp == '/')
    {
      ...
      fmt = decode_format (&tmp, last_format, last_size, (enum type_instance_flag_value) 0);
      exp = (char *) tmp;
    }
  ...
  if (exp != 0 && *exp != 0)
	...
	next_address = value_as_address (val);
      ...
      if (TYPE_CUDA_ALL(type))
        {
	  fmt.segment_type = (enum type_instance_flag_value) (TYPE_INSTANCE_FLAGS(type)
...
static void
map_command (const char *arg, int from_tty)
{
  char *exp = NULL;
  ...
  expression_up expr = parse_expression (exp);
  expression_up fexpr = parse_expression (function);
  ...
  if (type &&
      TYPE_CODE (type) == TYPE_CODE_ARRAY)
    {
	  ...
	  set_internalvar (lookup_internalvar ("value"),
		           subscript);
          fval = evaluate_expression (fexpr.get ());
