...
static bool module_name_matcher (const char *name)
{
  return strstr (name, "::") != NULL;
}

static bool file_name_matcher (const char *name, bool basenames)
{
  return 1;
}
...
static void
modules_info (const char *ignore, int from_tty)
{
  int first = 1;
                         ...
                         const enum search_domain kind, struct objfile *objfile)
{
  ...
  lookup_name_info lookup_name (module->name, symbol_name_match_type::FULL);
...
static void
module_symbol_info (const char *module_name, enum search_domain kind, int from_tty)
{
...
static void
module_functions (const char *module_name, int from_tty)
{
...
static void
module_variables (const char *module_name, int from_tty)
{
