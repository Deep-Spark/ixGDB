 ...
 */
static bool
cuda_frame_inlined_p (struct frame_info *next_frame)
{
  int num_inlined_frames = 0;
  CORE_ADDR pc = 0;
  struct block *block = NULL;
  struct block *this_frame_block = NULL;
  struct block *outermost_block = NULL;
...
static CORE_ADDR
cuda_abi_frame_cache_base (struct frame_info *this_frame)
{
...
static CORE_ADDR
cuda_noabi_frame_cache_base (struct frame_info *this_frame)
{
  return 0;
}

static struct cuda_frame_cache *
cuda_frame_cache (struct frame_info *this_frame, void **this_cache)
{
...
/* cuda_frame_base_address is not ABI-dependent, since it only
   ...
   compilations. */
static CORE_ADDR
cuda_frame_base_address (struct frame_info *this_frame, void **this_cache)
{
  struct cuda_frame_cache *cache;
  CORE_ADDR base;
...
cuda_abi_frame_id_build (struct frame_info *this_frame, void **this_cache,
...
static struct frame_id
cuda_noabi_frame_id_build (struct frame_info *this_frame, void **this_cache,
                           struct frame_id *this_id)
{
  struct cuda_frame_cache *cache;
